...
void free_command(Command *command) {
    while (command) {
        ...
        if (command->is_a_string && command->val_a.str_val != NULL) {
            free(command->val_a.str_val);
            command->val_a.str_val = NULL;
        }
        if (command->is_b_string && command->val_b.str_val != NULL) {
            free(command->val_b.str_val);
            command->val_b.str_val = NULL;
        }
        free(command);
